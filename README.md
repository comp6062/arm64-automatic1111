...
- Enables API support by default
...
- Runs with `--listen` so other devices on your network can access the WebUI
- Shows the URL with your local IP
- API is enabled

Mode 2 — Offline mode:

- Runs strictly on `127.0.0.1:7860`
- No external access
- API is enabled
...
- Removes `run_sd.sh`
- Does not remove the Outpaint Helper
...
The launcher starts A1111 with `--api` enabled.
...
- A unified launcher script with API support
